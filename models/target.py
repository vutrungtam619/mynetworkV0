...
class Target:
    ...
    def get_heatmap(self, batch_gt_bboxes, batch_size, device, sigma_factor=3):
        r = max(1, int(min(0.6 / self.vx, 0.6 / self.vy) / 2))
        gaussian = self._get_gaussian_kernel(r, r / sigma_factor, device)
        ...
        for b in range(batch_size):
            ...
            for x, y in zip(cx.tolist(), cy.tolist()):
                ...
                heatmap[b, 0, ty:by, lx:rx] = torch.max(
                    heatmap[b, 0, ty:by, lx:rx],
                    gaussian[gt_:gb, gl:gr],
                )
